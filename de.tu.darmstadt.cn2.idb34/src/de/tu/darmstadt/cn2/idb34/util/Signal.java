...
public class Signal {
    ...
    public static double getAverageBrightness(Integer[][] bitmapMatrixA) {
        BigDecimal avg = new BigDecimal(0);
        for (int i = 0; i < bitmapMatrixA.length; i++) {
            for (int j = 0; j < bitmapMatrixA[0].length; j++) {
                avg.add(new BigDecimal(bitmapMatrixA[i][j]));
            }
        }
